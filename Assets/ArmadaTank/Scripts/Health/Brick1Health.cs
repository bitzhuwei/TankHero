...
public class Brick1Health : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var parts = this.renderer.material.name.Split(emptySpace, System.StringSplitOptions.RemoveEmptyEntries);
        this.initialMaterial = parts[0];
        //Debug.Log(string.Format("brick material: {0}", parts[0]));
    }
    ...
    private bool GetDefenceRule(BulletBase bulletScript)
    {
        ...
        }
        {
            if (this.initialMaterial == MaterialName.strTerrWall_G_0
                || this.initialMaterial == MaterialName.strTerr_01)
            {
